...
const SearchBurials = ({
  selectedBurials: refSelectedBurials,
  selectedTour: refSelectedTour,
  hoveredIndex: refHoveredIndex,
  showAllBurials: refShowAllBurials,
  lat: refLat,
  lng: refLng,
  sectionFilter: refSectionFilter,
  overlayMaps: refOverlayMaps,
  updateSelectedBurials,
  updateSelectedTour,
  includesTour,
  updateSectionFilter,
  updateShowAllBurials,
  window,
}) => {
  ...
  const addToResults = useCallback(
    (burial) => {
      if (
        burial &&
        !selectedBurials.some((b) => b.OBJECTID === burial.OBJECTID)
      ) {
  ...
  const removeFromResults = useCallback((objectId) => {
    setSelectedBurials((prev) =>
      prev.filter((burial) => burial.OBJECTID !== objectId)
    );
    updateSelectedBurials((prev) =>
      prev.filter((burial) => burial.OBJECTID !== objectId)
    );
  }, []);
  ...
  const onLocateMarker = () => {
    ...
    const id = navigator.geolocation.watchPosition(
      (position) => {
        ...
        const bufferedBoundary = turf.buffer(boundaryPolygon, 8, {
          units: "kilometers",
        });
  ...
  const closeButtonStyle = {
    ...
    return (
      <div>
        <Paper elevation={3} className="left-sidebar">
          <Box sx={{ p: 2 }}>
            <Box sx={{ display: "flex", gap: 1 }}>
              {
                isMobile &&
                <button style={closeButtonStyle} onClick={handleOpen}>X</button>
              }
              <Autocomplete
                freeSolo
                ...
                onChange={(event, newValue) => {
                  ...
                  if (newValue && window.mapInstance) {
                    ...
                    const section = ARC_Sections.features.find(
                      (f) => f.properties.Section === newValue
                    );
                    if (section) {
            ...
            <Box sx={{ maxHeight: "calc(100vh - 200px)", overflow: "auto" }}>
              ...
              >
                <Typography variant="h6">
                  Search Results ({selectedBurials.length})
                </Typography>
  ...
  //   <>
  //       {!open? (
  //         <div style={arrowStyle} onClick={handleOpen}>▶</div>
  //       ):(
  //         <SearchWindow/>
  //       )}
  //   </>
